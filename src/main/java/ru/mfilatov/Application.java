...
public class Application {
    ...
    public static void main(String[] args) {
        ...
        var julianDate = julianDayCalculator.getJulianDayNumberEdwardGrahamRichards(2024, 3, 14) - longitude / (15 * 24.0);
        ...
        var computedTimes = computePrayerTimes(defaultTimes, CalculationMethods.RUSSIA, julianDate, latitude);
    ...
    public static Times computePrayerTimes(Times times, CalculationMethods method, double julianDate, double latitude) {
        ...
        var maghrib = sunAngleTime(method.getTimes().maghrib(), portionTimes.maghrib(), julianDate, false);
    ...
    public static double sunAngleTime(double angle, double time, double julianDate, boolean directionCCW){
        ...
        var t = 1 / 15.0 * acos((-sin(angle) - sin(declination) * sin(latitude)) / (cos(declination) * cos(latitude)));
    ...
    public static Times adjustTimes(Times times, double timeZone, double  longitude) {
        ...
        return new Times(times.imsak() + tzAdjust, times.fajr() + tzAdjust, times.sunrise() + tzAdjust, times.dhuhr() + tzAdjust, times.asr() + tzAdjust, times.sunset() + tzAdjust, times.maghrib() + tzAdjust, times.isha() + tzAdjust);
